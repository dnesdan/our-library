...
function Reader({ book, initialPage, onPage, onClose }: { book: Book; initialPage: number; onPage: (page: number) => void; onClose: () => void }) {
  ...
  useEffect(() => { const key = (e: KeyboardEvent) => { if (e.key === "ArrowLeft") go(page - 1); if (e.key === "ArrowRight") go(page + 1); if (e.key === "Escape") settings || toc ? (setSettings(false), setToc(false)) : onClose(); }; window.addEventListener("keydown", key); return () => window.removeEventListener("keydown", key); }, [go, onClose, page, settings, toc]);
  const chapter = readerPages[page], readProgress = Math.round(((page + 1) / readerPages.length) * 100);
  return <div className={`reader reader-${readerTheme} font-${font} width-${width} mode-${mode}`} style={{ "--reader-size": `${fontSize}px`, "--line-height": lineHeight, "--align": align } as React.CSSProperties} onTouchStart={e => touchX.current = e.touches[0].clientX} onTouchEnd={e => { const d = e.changedTouches[0].clientX - touchX.current; if (Math.abs(d) > 55) go(page + (d < 0 ? 1 : -1)); }}>
    ...
    <button className="reader-zone reader-left" onClick={() => go(page - 1)} aria-label="Previous page" /><main className="reader-page" onClick={() => setControls(!controls)}><div className="chapter-kicker">{String(page + 1).padStart(2, "0")}</div><h1>{chapter[0]}</h1><p className="lead">{chapter[1]}</p><p>{chapter[2]}</p><p>{chapter[3]}</p>{mode === "scroll" && readerPages.slice(page + 1).map((c, i) => <section key={i}><h1>{c[0]}</h1><p className="lead">{c[1]}</p><p>{c[2]}</p><p>{c[3]}</p></section>)}</main><button className="reader-zone reader-right" onClick={() => go(page + 1)} aria-label="Next page" />
...
export default function LibraryApp() {
  ...
  const navigateRoom = useCallback((direction: "left" | "right" | "up" | "down") => {
    ...
    const coords = (index: number) => ({ row: Math.floor(index / 4) % 4, x: [-5.25,-1.75,1.75,5.25][index % 4] - .66 + Math.floor(index / 16) * .62 });
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      if (view !== "room" || readerBook || selectedId || searchOpen || profileOpen || filterOpen || (event.target instanceof HTMLInputElement)) return;
      ...
      if (event.key === "Enter" && roomBook) { event.preventDefault(); setSelectedId(roomBook.id); setRoomSelected(null); }
    };
    window.addEventListener("keydown", key); return () => window.removeEventListener("keydown", key);
  }, [filterOpen, navigateRoom, profileOpen, readerBook, roomBook, searchOpen, selectedId, view]);
  ...
  return <div className={`app theme-${theme} view-${view}`}>
    ...
    {view === "catalog" ? <main className="catalog"><section className="welcome-row"><div><span className="eyebrow">Saturday, quiet morning</span><h1>{profile === "Dan" ? "Good morning, Dan." : "Good morning, Lucia."}</h1><p>{continueBooks.length ? `You have ${continueBooks.length} stories waiting for you.` : "What shall we read next?"}</p></div><button className="room-invite" onClick={enterRoom}><span><small>Step inside</small><strong>Our room</strong></span><i>→</i></button></section>
      ...
      <section className="shelf-section all-books"><div className="section-heading browse-heading"><div><h2>{filter === "All" ? "Your shelves" : filter}</h2><span>{filtered.length} books</span></div><button className="filter-trigger" onClick={() => setFilterOpen(!filterOpen)}><SlidersHorizontal size={16} />{filter}<ChevronDown size={14} /></button></div><div className="filter-bar" aria-label="Book filters">{FILTERS.map(f => <button key={f} className={filter === f ? "active" : ""} onClick={() => setFilter(f)}>{f}</button>)}</div>{filtered.length ? <div className="book-grid">{filtered.map((book, i) => <BookCard key={book.id} book={book} profile={profile} onOpen={() => setSelectedId(book.id)} index={i} />)}</div> : <div className="empty-state"><Search size={24} /><h3>No book found on these shelves</h3><p>Try a title, author, genre, or another filter.</p><button onClick={() => { setQuery(""); setFilter("All"); }}>Show all books</button></div>}</section>
      ...
      <LibraryRoom books={personalizedBooks} theme={theme} selectedId={roomSelected || roomSearchMatch?.id || null} search={searchOpen ? query : ""} profile={profile} onSelect={setRoomSelected} onClear={() => setRoomSelected(null)} />
      ...
      <div className={`room-book-navigation ${roomBook ? "is-selected" : ""}`} aria-label="Browse books on the shelves">
        <button onClick={() => navigateRoom("left")} aria-label="Previous book"><ArrowLeft size={18} /></button>
        <div aria-live="polite"><small>{roomBook ? `${roomBooksInOrder.findIndex(book => book.id === roomBook.id)+1} of ${roomBooksInOrder.length}` : "Explore the shelves"}</small><strong>{roomBook?.title || "Choose a book"}</strong></div>
        <button onClick={() => navigateRoom("right")} aria-label="Next book"><ArrowRight size={18} /></button>
      </div>
      {roomBook && <div className="room-selection"><div className="room-mini-cover"><Cover book={roomBook} compact /></div><div><small>{roomBook.highShelf ? "The ladder found it" : "From our shelves"}</small><strong>{roomBook.title}</strong><span>{roomBook.author}</span></div><button onClick={() => { setSelectedId(roomBook.id); setRoomSelected(null); }}>Open book <ArrowRight size={16} /></button><button className="room-selection-close" onClick={() => setRoomSelected(null)} aria-label="Put book back"><X size={17} /></button></div>}
    ...
    <nav className="mobile-nav"><button className={view === "catalog" ? "active" : ""} onClick={() => setView("catalog")}><Library size={20} /><span>Library</span></button><button className={view === "room" ? "active" : ""} onClick={enterRoom}><span className="door-icon" /><span>Our room</span></button><button onClick={() => setSearchOpen(true)}><Search size={20} /><span>Search</span></button><button onClick={() => setProfileOpen(!profileOpen)}><span className="nav-avatar">{profile[0]}</span><span>{profile}</span></button></nav>
  </div>;
}
